...
<template>
  <div class="fullPage bg-[var(--color-main-bg)] pb-[20px]">
    ...
    <div class="w-full h-auto sticky top-0 z-10">
      <!-- 下载模块 -->
      <div
        class="bg-[var(--color-main-bg)] flex items-center w-full overflow-hidden relative h-[3.125rem] px-[.75rem]"
      >
        <div
          class="absolute top-0 left-0 w-[3.9rem] h-[3.9rem] bg-[rgba(255,255,255,0.1)] transform -translate-x-1/2 -translate-y-1/2 rounded-[100px]"
        ></div>
        <div
          class="absolute top-[.25rem] left-[.25rem] text-[.875rem] text-white"
        >
          <svg
            fill="currentColor"
            width="1em"
      ...
      <!-- header -->
      <div
        class="flex h-[3.125rem] items-center px-[.75rem] bg-[var(--color-tabbar)]"
      >
        ...
        <img
          class="w-[7.5rem] h-auto ml-[1rem]"
          src="/variable/logo.png"
          alt=""
        />
    ...
    <div class="flex justify-between px-[.75rem] mt-[.75rem]">
      ...
      <div class="flex flex-col items-center justify-between">
        <div
          class="rounded-[.875rem] w-[11rem] overflow-hidden animate-float-2"
        >
          <img class="w-full h-auto" src="/static/gift2.gif" alt="" />
        </div>
        <div
          class="rounded-[.875rem] w-[11rem] overflow-hidden animate-float-3"
        >
    ...
    <div class="flex mt-[1.56rem] px-[.75rem] items-center">
      ...
      >
        ...
        <div class="marquee w-full ml-[.5rem]">
          <div class="marquee-track">
            <span
              ...
              >Código de resgate em 20 de outubro: 10188 Recompensa máxima: R$
        ...
        <p
          class="bg-[var(--color-active)] px-[.5rem] py-[.25rem] rounded-[.5rem] font-[600] text-[.75rem] text-white"
        >
          7
        </p>
      </div>
      ...
      <div class="w-full overflow-x-auto hide-scrollbar rounded-[.875rem]">
        <div class="flex flex-nowrap gap-[.625rem] w-max">
          ...
          >
            ...
            >
              <div
                class="w-[1.5rem] h-[1.5rem] bg-[length:800%_400%] bg-[url('/static/game_platform.png')]"
                :style="{ backgroundPosition: `${item.posX} ${item.posY}` }"
              ></div>
            </div>
            <p
              class="text-center text-white text-[.875rem] font-[600] truncate"
            >
              {{ item.label }}
            </p>
            <p class="text-center text-[#FFFFFF66] text-[.625rem] font-[400]">
    ...
    <div v-for="(item, idx) in 2" :key="idx">
      <div
        class="flex justify-between items-center px-[.75rem] mt-[1.75rem] mb-[.9375rem]"
      >
      ...
      <div class="flex gap-[.75rem] flex-wrap px-[.75rem]">
        <div
          class="relative w-[calc(25%-0.5625rem)] rounded-[12.5%_/_9.25%] overflow-hidden"
          v-for="(item, idx) in 16"
          :key="idx"
        >
          <img
            src="https://game-logo.6-q-r-s.com/prod/style1/en/PG_FortuneTiger.jpg"
            alt=""
          />
          <!-- 收藏角标 -->
          <div
            class="text-[#ff8e04] absolute top-[.5rem] right-[.5rem] cursor-pointer"
            @click="toggleFav(idx)"
            :aria-pressed="isFav(idx)"
            :title="isFav(idx) ? '已收藏' : '未收藏'"
          >
            <svg
              class="w-[.875rem] h-[.875rem] text-current"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              ...
              <g clip-path="url(#clip)">
                ...
                <path
                  ...
                  :fill="isFav(idx) ? 'currentColor' : 'none'"
                  fill-opacity="0.93"
                />
    ...
    <div class="bg-[#322071] bg-[url('/static/home_footer_bg.png')] bg-cover bg-center mt-[1rem] pt-[1.375rem] pb-[2.25rem] px-[.75rem]">
      <img src="/variable/logo.png" class="h-[2.25rem] w-auto" alt="">
       <div class="relative w-[7.75rem] h-[0.125rem] rounded opacity-20 mt-[1rem]" 
      ...
      <p class="text-[.75rem] font-[400] text-white opacity-40 text-left mt-[2.25rem]">
        58ff.COM é operada pela 58ff N.V., registrada sob o número 18958, com sede em Zuikertuintjeweg Z/N (Zuikertuin Tower), Curaçao. Licenciada e autorizada pelo Governo de Curaçao sob a Master License of Gaming Services Provider N.V., Licença Nº GLH-OCCHKTW07878965325.
        ...
        3.Ganhos ou perdas em jogos sociais não garantem resultados semelhantes em jogos com apostas em dinheiro real no futuro.
      </p>
    </div>
